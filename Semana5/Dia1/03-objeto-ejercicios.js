...
function peliculaPopular() {
  let mayorvalorPopularidad = 0;
  let mayor = -1;
  for (let i = 0; i < peliculas.length; i++) {
    if (mayorvalorPopularidad < peliculas[i].popularity) {
      mayorvalorPopularidad = peliculas[i].popularity;
      mayor = i;
    }
  }
  return (
    peliculas[mayor].title + " con una popularidad de " + mayorvalorPopularidad
  );
}

console.log("Pelicula más popular", peliculaPopular());
...
function peliculaConMasGeneros() {
  ...
  let mayor = -1;
  for (let i = 0; i < peliculas.length; i++) {
    if (mayorGeneros < peliculas[i].genre_ids.length) {
      mayorGeneros = peliculas[i].genre_ids.length;
      mayor = i;
    }
  }
  return (
    peliculas[mayor].title + " con una cantidad de generos de " + mayorGeneros
  );
}
console.log("Pelicula con más generos es", peliculaConMasGeneros());
// Ejercicio 4:
// Función que imprime los géneros a los que pertenece la primero película
